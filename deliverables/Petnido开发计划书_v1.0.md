...
数据库已有 `User`、`Profile`、`Pet`、`Need`、`NeedPet`、`ServiceProfile`、`Service`、`Application`、`Booking`、`Message` 等基础模型；但收藏、位置档案、会话聚合、服务关联预约、通知偏好、不可变快照、寄养容量控制等仍不完整。
...
| 地点 | 已有经纬度、地址输入、地图与地理编码组件 | 复用地图选点能力；取消精确地址采集，只保存位置经纬度和非精确显示标签 |
...
| 3 | 个人资料 | `Profile` 只有 `bio/isSitter/isOwner`，头像昵称主要在 `User` | 头像、昵称、宠物档案和地图位置档案 | 字段边界不清，缺可复用位置和完整资料状态 | 重新确定 `User/Profile` 边界，迁移已有 name/image |
| 4 | 位置 | 需求和服务当前存 `addressRaw` 与经纬度，现有表单会提示填写建筑物/房号 | 不采集楼号、楼层、房号等精确地址；只在地图选点并保存经纬度 | 当前地址文本采集与已确认隐私规则直接冲突 | `REBUILD` 为位置档案；移除精确地址输入，迁移时丢弃或隔离旧精确文本，不在新流程展示 |
...
| 11 | 需求状态/过期 | 数据库有 OPEN/MATCHED/CLOSED/CANCELLED；`EXPIRED` 只存在展示类型 | 需求到达 `endDate` 后自动从公开列表移除，个人列表和收藏仍可见 | 所有读取路径尚未统一按结束时间判断 | 保留现有主要状态；令 `expiresAt = endDate` 或直接以 `endDate` 查询；是否持久化 `EXPIRED` 仅是技术实现选择 |
| 12 | 需求公开查询 | 已有 `listAll` 和公开页面；详情查询当前为 protected | 游客可浏览列表和详情，只有动作要求登录 | 详情权限与目标冲突；旧字段可能包含不应继续使用的地址文本 | 拆成 `getPublic` 和 `getMine`；新 DTO 只返回地图坐标及非精确显示标签 |
...
| 24 | 预约容量 | 当前 Service 无最大数量，Booking 无宠物数量和可用性锁定 | 只有寄养服务设置最大接待宠物数，并按预约宠物数占用容量 | 无法计算、无法防止寄养超额接待 | 仅为寄养服务新增宠物容量与预约宠物快照；事务/条件更新 + 并发测试 |
...
| 33 | 隐私 | Need 存地址文本，现有常量甚至提示填写房号 | 产品不采集精确家庭地址，只保存用户地图选点经纬度 | 现有采集方式违反目标数据最小化规则 | 删除精确地址输入与存储路径；旧字段停止写入并制定清理方案 |
...
| UX-07 | 发布流程修改地图位置后如何更新档案 | 已确认 | 默认位置可能被意外覆盖 | P0 | 发布成功后将本次地图选点更新为该档案默认位置；发布前只保存在草稿中 |
...
| UX-26 | 无障碍、低带宽、地图不可用和图片上传失败的替代流程未实现 | 已确认 | 部分用户无法完成核心动作 | P0 | 支持区域/地名搜索后选点但只保存坐标；图片可跳过；补键盘与错误恢复 |
...
| DATA-05 | 旧模型保留地址文本且前端采集建筑物/房号 | 已确认 | 违反“不收集精确地址”的产品决定 | P0 | 新模型只保存经纬度、可选非精确地名和显示精度；停止写入并清理旧精确文本 |
| DATA-06 | 用户档案和服务档案的地点字段含义重叠 | 已确认 | 默认值来源不唯一 | P0 | 明确个人默认地图位置与接单默认区域 |
...
| DATA-11 | 当前过期规则没有在所有查询中统一使用 `endDate` | 已确认 | 需求结束后仍可能公开 | P0 | 明确 `expiresAt = endDate`；可持久化派生值但不得与 `endDate` 不一致 |
...
| API-09 | 寄养预约容量确认无并发保护 | 已确认 | 寄养家庭超额接待 | P0 | 寄养预约使用事务隔离/锁/条件计数策略与压测；其他模式不计算宠物容量 |
...
| API-13 | 地理编码第三方失败、配额、缓存和超时处理不足 | 已确认 | 发布流程卡死 | P0 | 超时、缓存、区域搜索/手动选点和供应商抽象；搜索文字不持久化为精确地址 |
...
| FE-09 | 日期、币种、地图位置组件尚未完全共享统一格式规则 | 已确认 | 页面显示不一致 | P0 | 建立 formatters 和 design-system wrappers |
...
| QA-08 | 缺数据最小化与公开 DTO 测试 | 已确认 | 旧地址文本或不必要资料可能被接口返回 | P0 | Contract test 确认新请求、响应和数据库均不含楼号、楼层、房号字段 |
...
以下决定已作为开发基线，不再保留为开放问题。后续若改变，必须进入产品变更评审：

1. 需求在需求结束时间过期，即 `expiresAt = endDate`。公开查询、匹配和应聘校验均以 `endDate > now()` 为准。
2. 需求取消已选服务者后保留原应聘历史；需求未结束时重新公开，原服务者可以新建一次应聘，不能覆盖旧记录。
3. 只有寄养服务设置预约容量；容量含义为寄养家庭一次可接待的宠物总数，按预约中的宠物数量占用。上门和自定义服务不使用宠物数容量。
4. 服务暂停只阻止新预约，不自动取消已有 `PENDING` 或 `CONFIRMED` 预约；历史预约由双方单独处理。
5. 批量修改地点/币种前展示受影响服务数量；确认后更新服务当前版本和服务档案默认值，不修改已有预约快照。
6. 平台不收集、保存或展示楼号、楼层、房号等精确家庭地址，也不设置“确认后开放精确地址”的流程。地点只通过地图选点获得经纬度，用于显示用户选择的位置、距离筛选和匹配；可保存非精确的城市/区域显示标签，但不得反向保存门牌级文本。
7. 咨询后再应聘/预约时，同一对象、同一双方复用同一会话；系统消息记录动作转换。
8. 同一有效状态下禁止重复应聘或重复预约；取消后允许创建新的 attempt，并保留历史记录。
9. 需求接受一名服务者后，其他待处理应聘显示“需求已结束/已找到服务者”，不把它表述为对个人能力的拒绝。
10. 服务价格和需求预算允许固定金额、区间或面议；筛选分别使用固定金额、区间上限或服务起价，并在列表明确类型。
11. 交通费和物资费结构化记录为包含、另计固定额、实报实销或协商，并在预览与详情中单独展示。
12. 首发优惠只支持简单固定优惠说明或数量阶梯，不做优惠码、组合券和复杂叠加。
13. 宠物健康、过敏和用药信息属于完成照护所需的普通受保护资料，不按精确住址或正式医疗档案的高敏级别处理。信息由用户自愿填写，仅在宠物档案、相关需求及已参与业务的用户界面按需展示；不写入公开搜索摘要、邮件正文或普通日志。
14. 账号删除后保留必要业务记录并匿名化个人身份；具体保存期限按适用地区法规在上线前完成确认。
15. 三语内容确定一个 source locale，其他两种语言由人工审核；技术上所有语言 key 必须同步。
16. 知识链接必须记录来源、语言和免责声明，并指定内容维护责任人；首发可使用静态数据源。
17. 服务者可以在没有公开服务时保持接单模式，但公开服务者列表只展示至少有一项开启服务的用户。
18. 服务详情以每项服务自己的地图位置快照为准；服务档案地点只作为新服务默认值。
19. 需求或服务修改后，已发起的应聘/预约保留创建时快照，并提示源内容已更新。
20. P0 草稿只做本地保存，不要求跨设备同步；服务端 Draft 列为后续能力。
...
- 个人资料、宠物档案、地图位置档案。
- 服务档案、接单模式和服务默认地图位置/货币。
- 上门、寄养、自定义三类需求发布。
- 上门、寄养、自定义三类服务发布。
- 公开需求、服务者、服务浏览与筛选。
- 收藏、咨询、站内聊天。
- 应聘与需求确认状态机。
- 预约、时间段、确认状态机，以及寄养服务的宠物容量。
- 需求自动过期及公开可见性规则。
- 邮箱通知开关、邮箱验证及消息邮件。
- 发布后匹配与降级推荐。
- 中文、英文、日文切换。
...
7. 搜索和业务接口均不采集或返回门牌级地址；位置数据只包含用户地图选点经纬度和非精确显示标签。
...
| `UserLocation` | 标签、经纬度、非精确区域显示名、是否默认 | 不保存楼号、楼层、房号等精确地址文本；同一用户最多一个默认位置 |
...
| `Service` | 模式、地点/货币快照、状态、价格规则 | 多服务独立开关、编辑和删除；寄养明细单独保存最大宠物容量 |
...
| `Booking` | 服务、预约人、服务者、时间范围、宠物快照、状态、会话 | 寄养成功预约按宠物数量占用容量；其他模式不使用宠物数容量 |
...
- `Booking(serviceId, startAt, endAt, status)`：时间冲突查询；寄养服务额外按宠物数量汇总容量。
...
服务详情显示指定日期已有的成功预约数量，不暴露其他预约人的身份。确认操作必须重新检查服务状态和时间可用性；仅寄养服务额外检查最大接待宠物数。
...
| 地点 | 通用 | 默认使用个人地图位置，可修改；没有则在地图选点；不提供精确地址文本输入 |
| 预算 | 通用 | 币种、金额、交通费/物资费规则 |
| 预览确认 | 通用 | 隐私提示、快照预览、发布 |
| 成功页 | 通用 | 提交后事务保存需求、宠物档案和地图位置，再进行邮箱提醒和推荐 |
...
| 寄养环境 | 寄养 | 地图位置、环境照片、已有宠物、物资、条件和最大宠物数 |
...
| `/dashboard/locations` | 位置档案 | 地图选点、新建、编辑、默认位置；不采集门牌、楼层或房号 |
...
| `locationProfile` | `listMine`、`create`、`update`、`setDefault`、`delete` | 仅保存经纬度和非精确区域显示信息的位置档案 |
...
| `booking` | `getAvailability`、`getDailyConfirmedCount`、`create`、`confirm`、`decline`、`cancel`、`complete` | 通用预约；寄养模式附加宠物容量校验 |
...
1. 校验草稿、时间、金额、地图坐标和模式专属字段；拒绝门牌、楼层、房号等精确地址文本字段。
2. 创建或更新用户流程内新增的宠物档案。
3. 创建或更新地图位置，并按用户选择设置默认位置。
4. 创建需求及宠物、位置、任务不可变快照；写入模型不得包含精确地址文本。
...
1. 校验服务档案已启用接单模式。
2. 校验三类服务字段、可用时间和价格；只有寄养服务校验最大宠物容量。
3. 创建服务及地点、货币、规则快照。
4. 更新服务档案默认地点和默认货币。
...
1. 校验服务仍开启且操作者为服务者。
2. 重新计算预约时段内已确认宠物数量。
3. 校验可用时间和例外日期；若为寄养服务，再校验最大宠物容量。
4. 以条件更新确认预约；寄养并发冲突时返回“容量刚刚发生变化”。
5. 写入系统消息、站内通知和邮件事件。
...
2. 查询该日期成功预约数量；寄养服务同时显示剩余宠物容量。
3. 创建会话、首条消息和 `PENDING` 预约。
4. 服务者确认时再次校验可用性；寄养服务同时进行并发宠物容量校验。
5. 确认后双方看到成功状态；取消确认后，寄养预约释放宠物容量并发送系统消息。
...
邮件正文只显示必要摘要和站内跳转链接，不包含精确坐标、宠物健康/用药详情等非必要内容。
...
- 日志不得写入完整经纬度、验证码、消息全文和认证令牌。

### 13.2 位置隐私

- 平台不采集或保存楼号、楼层、房号等精确地址文本。
- 页面通过用户选择的地图坐标显示位置；列表可使用城市/区域标签，地图标记不得反向展示门牌级地址。
- 完整经纬度不进入普通日志、分析事件和邮件正文。
...
3. 发布需求时从已有宠物档案选择宠物，从默认地图位置读取并修改；成功后档案正确更新，需求保留位置快照。
4. 无宠物/位置用户在流程中创建；发布失败时正式档案不产生半成品，所有请求和数据库记录均不包含门牌、楼层或房号字段。
...
- 增加需求模式专属明细、任务、宠物和地图位置快照。
- 重构分步表单，按自然步骤读取宠物与地图位置档案。
- 实现发布事务、草稿、编辑和预览。
- 完成上门模式与自定义模式。
...
**验收**：寄养宠物容量永不超额；暂停服务不能产生新预约；取消寄养预约后宠物容量立即恢复。
...
| 寄养预约容量超额 | 寄养家庭收到超出接待宠物数的确认 | 事务、条件更新、唯一约束和并发测试 |
| 需求过期任务延迟 | 过期内容仍被看到 | 定时标记 + 所有公开查询强制时间过滤 |
| 旧精确地址文本继续流入新系统 | 违反数据最小化要求 | 新流程不采集、API 拒绝相关字段、旧列停止写入并制定清理方案 |
| 邮件服务不稳定 | 用户错过消息 | Outbox、重试、站内通知为事实来源 |
| 三语文案不同步 | 流程不可用或含混 | CI 检查 key，核心流程逐语 E2E，产品校对 |
| 地图/地理编码配额或准确度 | 位置选择失败 | 缓存、区域搜索后手动选点、可替换供应商接口层；只持久化坐标 |
...
| 宠物/地图位置自然步骤复用，成功后保存 | E2、E3 | 发布事务与无精确地址 Contract Test |
...
| 预约时间、成功数量、寄养宠物容量 | E8 | 寄养并发容量 E2E |
...
- [ ] 三语文案负责人和审核流程已确定。
- [x] 需求过期时间确定为需求结束时间。
- [x] 只有寄养服务使用容量，按预约宠物数量计算。
- [x] 平台不采集精确地址，只保存地图经纬度和非精确显示标签。
- [x] 宠物健康/用药资料按普通受保护照护资料处理，不进入公开摘要、邮件或日志。
- [ ] 功能开关、灰度发布和回滚负责人已确定。
