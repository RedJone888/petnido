...
export function profileEditorValueToPetInput(
  ...
  return {
    ...
    type: value.type as "DOG" | "CAT" | "RABBIT" | "BIRD" | "OTHER",
    customType: value.type === "OTHER" ? value.customType.trim() || null : null,
    quantity: pet.quantity,
...
export function StepPets({
  ...
  const taskForm = t.core.needPublishingTaskForm;
  ...
  const profileChoiceCopy = (
    ...
    } satisfies Record<Lang, PetProfileChoiceCopy>
  )[lang];
    ...
    <div className="space-y-5">
      <section className="relative z-30 flex flex-wrap items-center gap-3">
        ...
        {showValidation && !pets.length ? (
          <span
            role="alert"
            className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-danger-border bg-danger-bg px-3.5 text-sm font-bold text-danger-text"
          >
...
export function PetProfileQuestion({
  ...
  return (
    <label className="group inline-flex cursor-pointer items-center gap-2 text-sm font-semibold text-slate-600 transition focus-within:ring-2 focus-within:ring-primary/30 focus-within:ring-offset-2">
      <input
        type="radio"
        name={name}
        checked={checked}
