...
export function PendingBookingAction({ publicId, returnTo, idempotencyKey }: { publicId: string; returnTo: string; idempotencyKey: string }) {
  ...
  const booking = trpc.serviceBooking.create.useMutation({ onSuccess: (result) => router.replace(`/dashboard/notifications?view=conversations&conversation=${encodeURIComponent(result.conversationId)}`) });
