...
export function pendingActionSecret() {
  const secret =
    process.env.PENDING_ACTION_SECRET || process.env.NEXTAUTH_SECRET;
  if (!secret || secret.length < 32) {
    throw new Error("PENDING_ACTION_SECRET_MISSING");
  }
  return secret;
}
...
export function createPendingActionToken(
  ...
  const ttlSeconds = Math.min(
    Math.max(input.ttlSeconds ?? 15 * 60, 60),
    maxLifetimeSeconds,
  );
...
export function verifyPendingActionToken(
  ...
  const expectedSignature = signature(
    encoded,
    options?.secret ?? pendingActionSecret(),
  );
