...
function RoutineRow({
  ...
  const lang = usePageLanguage();
  const label = localizeTaskLabel(routine.label, lang, {
    templateId: routine.taskCode,
    custom: routine.custom,
  });
  const Icon = taskIcon(label);
  return (
...
function TaskRow({
  ...
  const lang = usePageLanguage();
  const label = localizeTaskLabel(task.label, lang, {
    templateId: task.taskCode,
    custom: task.custom,
  });
  if (compact) {
    return (
      <div className="flex items-start gap-3 py-2.5 text-sm">
        ...
        <div className="min-w-0">
          <p className="font-semibold text-[#302a34]">{label}</p>
          {task.notes && (
...
function buildRequestStory(request: NeedPreviewSnapshot, lang: Lang = "en") {
  ...
  const rawTasks =
    request.careType === "visit"
      ? (request.visit?.visits || []).flatMap((v) =>
          v.tasks.map((task) =>
      ...
      : request.careType === "boarding"
      ? (request.boarding?.routines || []).map((routine) =>
          localizeTaskLabel(routine.label, lang, {
            templateId: routine.taskCode,
            custom: routine.custom,
          }).trim(),
        )
      : (request.custom?.tasks || []).map((task) =>
          localizeTaskLabel(task.label, lang, {
            templateId: task.taskCode,
            custom: task.custom,
          }).trim(),
