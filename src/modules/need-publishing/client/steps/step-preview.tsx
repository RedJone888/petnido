      ...
      ? boardingRoutines.flatMap((config) =>
          config.routines.map((routine) => {
            const matchedPets = displayPets.filter(
              (p) =>
                routine.petIds.includes(p.id) ||
                displayPets.length === 1,
            );
            return {
              category: config.custom ? "CUSTOM" : (config.templateId ?? "CARE"),
              label: config.label,
              instructions: routine.instructions?.trim() || null,
              priority: routine.priority === "must" ? "MUST" : "NICE",
              scheduleKind: routine.scheduleType.toUpperCase().replaceAll("-", "_"),
              pets: matchedPets.map((p) => ({ name: p.name, petType: p.petType })),
            };
          }),
        )
      : tasks.map((task) => ({
