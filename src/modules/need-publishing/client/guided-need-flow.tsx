...
import { getNeedPublishingMessages, needPublishingMessages } from "@/modules/need-publishing/i18n/messages";
...
type NeedPublishingCopy = ReturnType<typeof getNeedPublishingMessages>["needPublishing"];

function buildScreens(
  careType: CareType | null,
  copy: NeedPublishingCopy,
  flowCopy: ReturnType<typeof getNeedPublishingMessages>["needPublishingClient"]["flow"],
): ScreenDefinition[] {
...
function emptyNeedModeDraft(): NeedModeDraftState {
  return {
    dates: {
      startDate: "",
      endDate: "",
...
export function GuidedNeedFlow({
  ...
  const switchCareType = useCallback(
    (next: CareType | null) => {
      ...
      if (careType) {
        setDraftByMode((items) => ({ ...items, [careType]: captureModeState() }));
      }
      const nextMode = draftByMode[next] ?? emptyNeedModeDraft();
      supplyCostDefaultAppliedRef.current = nextMode.supplyCostMode !== "discuss";
      travelCostDefaultAppliedRef.current = nextMode.budget.travelMode !== "none";
      ...
      setConfirmedScreenIds([...new Set([...preservedCommonIds, ...targetIds])]);
    },
    [
      careType,
      captureModeState,
  ...
  }, [authenticatedForDrafts, editingNeedId, resumeDraftId, serverResumeDraft.data, serverResumeDraft.error, serverResumeDraft.isLoading]);
  ...
  useEffect(() => {
    ...
    async function loadEditingNeed() {
      try {
        ...
        const draft = await beginEdit.mutateAsync(input).catch(() =>
          beginEdit.mutateAsync(input),
        );
        if (!active) return;
  ...
  const editingContentSignature = useMemo(() => {
    ...
    try {
      const content = JSON.parse(draftPersistenceSignature) as Record<string, unknown>;
  ...
  const screenSignature = (id: ScreenId) => {
    switch (id) {
      ...
      case "dates":
        return JSON.stringify({ dates, visitFrequency, customInterval, firstVisitDate, visitsPerDay, visitTimes, exactTimes });
      case "tasks":
        return JSON.stringify({ activeTasks, boardingRoutines, taskNotes });
      case "supplies":
        return JSON.stringify({ boardingSupplies, customBoardingSupplies, boardingSupplyNotes });
      case "requirements":
        return JSON.stringify({ boardingNeeds, customBoardingRequirements, boardingCompatibility, customHomeSituations, boardingHomeNotes, customNeeds, customWarnings, customRequirementsNotes });
      case "area":
        return JSON.stringify({ area, location, areaConfirmed, distance, transport, splitDirection });
      case "budget":
  ...
  }, [careType, currentId, draftReady, editingLoadError, frontierIndex, screens]);
  useEffect(() => {
    ...
    if (
      ...
      setConfirmedScreenIds(screens.filter((screen) => screen.id !== "preview").map((screen) => screen.id));
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [careType, confirmedScreenIds, draftReady, editingContentSignature, editingNeedId, screens]);
  ...
  const canPublish =
    ...
    (!editingNeedId || !editingDirty || (Boolean(serverDraftId) && !editingLoadError));
      ...
      case "frequency":
        return (
          <StepScheduleVisitTiming
            ...
            onCustomIntervalChange={setCustomInterval}
            showValidation={
              revealAllValidation || validationAttemptedScreenIds.has("frequency")
            }
            visitsPerDay={visitsPerDay}
            onVisitCountChange={changeVisitCount}
            visitTimes={visitTimes}
      ...
      case "area":
        return (
          <StepArea
            ...
            showValidation={revealAllValidation || validationAttemptedScreenIds.has("area")}
          />
        );
      case "budget":
        return (
  ...
  if (editingNeedId && !draftReady && !editingLoadError) {
